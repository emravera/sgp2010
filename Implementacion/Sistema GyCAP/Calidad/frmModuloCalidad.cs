...
{
    public partial class frmModuloCalidad : Form
    {
        private static frmModuloCalidad _frmModuloCalidad = null;
        private SplitterPanel areaTrabajo;
        private Panel activo = null;

        #region Inicio

        public frmModuloCalidad()
        {
            InitializeComponent();

            areaTrabajo = scUp.Panel2;
            flpMenu.AutoScroll = false;
            btnLineaMontaje.Tag = panelLineaMontaje;
            btnTemp.Tag = panelTemp;
            Size size = new Size(panelTemp.Size.Width, 0);
            panelLineaMontaje.Size = size;
            panelTemp.Size = size;
        }

        public static frmModuloCalidad Instancia
        {
            get
            {
                if (_frmModuloCalidad == null || _frmModuloCalidad.IsDisposed)
                {
                    _frmModuloCalidad = new frmModuloCalidad();
                }
                else
                {
                    _frmModuloCalidad.BringToFront();
                }
                return _frmModuloCalidad;
            }
            set
            {
                _frmModuloCalidad = value;
            }
        }

        private void frmfrmModuloCalidad_Load(object sender, EventArgs e)
        {
